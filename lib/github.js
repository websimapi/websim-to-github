...
async function createRepo(repoName, description) {
  try {
    ...
    if (msg && msg.includes('already exists')) {
      try {
        const user = loadAuth().github_user;
        const existing = await ghApi('GET', `/repos/${user}/${repoName}`);
...
async function repoHasCommits(owner, repoName) {
  try {
    const commits = await ghApi('GET', `/repos/${owner}/${repoName}/commits?per_page=1`);
