...
async function apiGet(path, cookie, params = {}, maxRetries = 4) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // After exhausting retries on rate limit, escalate to RateLimitError
      // so the pipeline can apply longer backoff instead of failing the project.
      if (status === 429) {
        if (attempt < maxRetries) {
          const wait = attempt * 4000;
          console.log(`[WebSim] 429 on ${path} — retry ${attempt}/${maxRetries - 1} in ${wait}ms`);
          await sleep(wait);
          continue;
        }
        const retryAfter = parseInt(err.response?.headers?.['retry-after']);
        throw new RateLimitError('WebSim', retryAfter > 0 ? retryAfter * 1000 : null);
      }

      // Retry on server/network errors
      if (attempt < maxRetries && (status === 500 || status === 503 || !status)) {
        const wait = attempt * 4000;
